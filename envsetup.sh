...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/narvis/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/narvis/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/narvis/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^narvis_") ; then
        NARVIS_BUILD=$(echo -n $1 | sed -e 's/^narvis_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $NARVIS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        NARVIS_BUILD=
    fi
    export NARVIS_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${NARVIS_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    NARVIS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/narvis/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch narvis_$target-userdebug
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the NARVIS GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/narvis/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/narvis/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                bacon|narvis|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function repopick() {
    T=$(gettop)
    $T/vendor/narvis/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $NARVIS_FIXUP_COMMON_OUT ]; then
...
. $ANDROID_BUILD_TOP/vendor/narvis/build/envsetup.sh
echo "======================================="
echo "      Narvis Build Environment       "
echo "=======================================" 
